...
CREATE TABLE `tb_local_auth` (
  ...
  KEY `fk_local_auth_profile` (`user_id`),
...
CREATE TABLE `tb_person_info` (
  ...
  `enable_status` int(2) NOT NULL DEFAULT '0' COMMENT '0禁止使用，1：允许使用',
  `user_type` int(2) NOT NULL DEFAULT '1' COMMENT '1:顾客，2店家3管理员',
  `create_time` datetime DEFAULT NULL,
  `last_edit_time` datetime DEFAULT NULL,
  PRIMARY KEY (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `tb_product` (
  ...
  KEY `fk_product_cate` (`product_category_id`),
  KEY `fk_product_shop` (`shop_id`),
  CONSTRAINT `fk_product_cate` FOREIGN KEY (`product_category_id`) REFERENCES `tb_product_category` (`product_category_id`),
...
CREATE TABLE `tb_shop_category` (
  `shop_category_id` int(100) NOT NULL AUTO_INCREMENT,
  `shop_category_name` varchar(100) DEFAULT '',
